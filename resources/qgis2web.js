...
var map = new ol.Map({
  controls: ol.control.defaults({ attribution: false }).extend([expandedAttribution]),
...
map.getView().fit([-14737863.909168, 1821705.236421, -8848031.785599, 8039965.710129], map.getSize());
map.getView().setCenter(ol.proj.fromLonLat([-98.5, 40]));
              ...
              if (layer.get("fieldImages")[currentFeatureKeys[i]] != "ExternalResource") {
                popupField +=
                  clusterFeature.get(currentFeatureKeys[i]) != null
                    ? autolinker.link(clusterFeature.get(currentFeatureKeys[i]).toLocaleString()) + "</td>"
                    : "";
              } else {
                popupField +=
                  clusterFeature.get(currentFeatureKeys[i]) != null
...
var onSingleClick = function (evt) {
  ...
  for (i = 0; i < wms_layers.length; i++) {
    if (wms_layers[i][1]) {
      var url = wms_layers[i][0].getSource().getGetFeatureInfoUrl(evt.coordinate, viewResolution, viewProjection, {
